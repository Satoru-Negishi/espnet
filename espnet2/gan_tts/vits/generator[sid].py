    ...
    def inference(
        ...
        if self.langs is not None:
            ...
            if g is None:
                ...
                g = g + g_
        # =================================
        #  mix_spk = {'spk1': 1~4, 'spk2': 1~4, 'ratio1': 0~1, 'ratio2': 0~1}
        if mix_spks is not None:
            g1 = self.global_emb(torch.tensor(mix_spks['spk1'])).unsqueeze(-1)
            g2 = self.global_emb(torch.tensor(mix_spks['spk2'])).unsqueeze(-1)
            g = g1 * mix_spks['ratio1'] + g2 * mix_spks['ratio2']
        #==================================

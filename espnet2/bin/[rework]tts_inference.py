...
class Text2Speech:
    ...
    def __init__(
        ...
        use_style_embedding: bool = False,
    ):
    ...
    def __call__(
        ...
        mix_sids:Optional[Dict] = None,
    ) -> Dict[str, torch.Tensor]:
        """Run text-to-speech."""
        assert check_argument_types()
        # check inputs
        if self.use_speech and speech is None and not self.use_style_embedding:
        ...
        if mix_sids is not None:
            batch.update(mix_sids=mix_sids)
        if self.use_style_embedding:
            batch.update(use_style_embedding=self.use_style_embedding)
    ...
    def from_pretrained(
        ...
        **kwargs: Optional[Any],
        ...
        if use_style_embedding:
            kwargs.update(use_style_embedding=use_style_embedding)
